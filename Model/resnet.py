...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000,deep_base=False,stem_width=32):
        ...
        self.bn1 = nn.BatchNorm2d(self.inplanes) #批量归一化
        ...
        self.avgpool = nn.AvgPool2d(7, stride=1)  #平均池化层，对输入的特征进行下采样
        self.fc = nn.Linear(512 * block.expansion, num_classes) #创建一个全连接层，全连接层的作用是将前一层的输出（通常是展平的特征向量）映射到最终的输出类别上。
