...
def connectivity_matrix(multimask, class_num):
    ...
    # batch = 1
    conn = torch.zeros([batch,class_num*8,rows, cols]).cuda() #一个全零的张量
    for i in range(class_num):
        mask = multimask[:,i,:,:]    #第0个通道的所有像素
        #print(mask.shape)
...
class connect_loss(nn.Module):
    def __init__(self,args,hori_translation,verti_translation,density=None,bin_wide=None):
        ...
        self.cross_entropy_loss = nn.CrossEntropyLoss(reduction='none')  #初始化一个交叉熵损失函数，用于多类别分类任务
        self.BCEloss = nn.BCELoss(reduction='none') #初始化一个二元交叉熵损失函数，用于二元分类任务
        self.dice_loss = dice_loss(bin_wide=bin_wide,density=density) #初始化一个Dice损失函数，这是一个常用于图像分割任务的损失函数，它衡量两个样本的相似度。
        self.bin_wide = bin_wide
        self.density = density
        self.args = args
    ...
    def single_class_forward(self, c_map, target):
        ...
        batch_num = c_map.shape[0] #获取一个名为 c_map 的数组或张量的第一维的大小，通常这个维度代表批次大小（batch size），即数据集中一次处理的样本数量。
        target = target.float()   #模型预测的目标值

        ### get your connectivity_mask ###
        con_target = connectivity_matrix(target,self.args.num_class)#(B, 8, H, W)

        ### matrix for shifting
        hori_translation = self.hori_translation.repeat(batch_num,1,1,1).cuda()  #张量扩展到批次大小，然后将其移动到GPU上
        verti_translation = self.verti_translation.repeat(batch_num,1,1,1).cuda()
        ...
        ### get edges gt###
        class_conn = con_target.view([c_map.shape[0],self.args.num_class,8,c_map.shape[2],c_map.shape[3]]) #用来表示每个类别在每个像素点的8邻域中的连通性信息
        sum_conn = torch.sum(class_conn,dim=2) #每个元素代表原始 class_conn 张量中对应位置的8个邻接点的总和。
        ...
        dice_l = self.dice_loss(pred[:,0], target[:,0])   #初始化一个Dice损失函数，这是一个常用于图像分割任务的损失函数，它衡量两个样本的相似度。
        # print(f"dice_l的值为：{dice_l}")

        bce_loss = self.BCEloss(pred, target).mean()   #初始化一个二元交叉熵损失函数，用于二元分类任务
        # print(f"bce_loss的值为：{bce_loss}")
        conn_l = self.BCEloss(c_map,con_target).mean() #初始化一个二元交叉熵损失函数，用于二元分类任务
        ...
        if self.args.dataset == 'chase':
            ...
            loss =  bce_loss + conn_l + edge_l + 0.2* bicon_l+dice_l #+ bce_loss# +loss_out_dice# +sum_l # + edge_l+loss_out_dice
              # 打印各个损失项的值
            # print(
            #     # f"BCE Loss: {bce_loss.item():.4f} Edge Loss: {edge_l.item():.4f} Bicon Loss: {bicon_l.item():.4f} Dice Loss: {dice_l.item():.4f} Total Loss: {loss.item():.4f}",
            #     # end="")
        return loss
