...
def ISIC2018_transform_newdata(sample, train_type):
    ...
    if train_type == 'train':
        ...
        # edges = randomflip_rotate(Image.fromarray(edges), Image.fromarray(edges), p=0.5, degrees=30)[0]
    else:
        image = image
        label = label
    ...
    # edges = ts.ToTensor()(edges)
    return {'image': image, 'label': label}

class ISIC2018_dataset(Dataset):
    def __init__(self, dataset_folder='/ISIC2018_npy_all_224_320',
        ...
        self.folder_file = '/home/ubuntu/Experiment/liyachao/DconnNet/DconnNet-mainT/data_loader/isic_datalist/' + folder
        ...
        if self.train_type in ['train', 'validation', 'test']:
            ...
            # self.edge = [join(dataset_folder, 'edges', x.split('.')[0] + '_segmentation.npy') for x in self.image_list]
        else:
