...
class PolypDataset(Dataset):
    ...
    def __init__(self, image_root, gt_root, trainsize, augmentations=True):
        ...
        self.gts = [os.path.join(gt_root, f) for f in os.listdir(gt_root) if f.lower().endswith(('.jpg', '.png'))]
...
class test_dataset:
    def __init__(self, image_root, gt_root, testsize,augmentations=True):
        ...
        self.size = len(self.images)
        self.augmentations = augmentations
        self.resize = transforms.Resize((self.testsize, self.testsize))
        self.rotate = RandomRotate(degrees=(0, 360))
        self.vf = RandomVerticalFlip(p=0.5)
        self.hf = RandomHorizontalFlip(p=0.5)
        self.img_transform = transforms.Compose([
